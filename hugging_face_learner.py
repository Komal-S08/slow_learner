# learner_app.py - Streamlit Dashboard with Cohere Integration (No OpenAI required)
...
def get_cohere_suggestions(student_profile_dict):
    prompt = f"""
    A student is struggling in academics. Profile:
    {student_profile_dict}

    Suggest 3 personalized, supportive, and practical remedial strategies to help the student improve.
    """
    try:
        ...
        return f"⚠️ Error getting Cohere suggestions: {e}"
...
with st.sidebar.form("student_form"):
    ...
    use_ai = st.checkbox("💬 Use AI Suggestions (via Cohere)", value=True)
...
if submit_btn:
    if scaler is None or model is None or feature_names is None:
        ...
        if use_ai:
            st.markdown("### 🤖 AI-Based Suggestions (Cohere)")
            student_data = {
            ...
            cohere_advice = get_cohere_suggestions(student_data)
            st.markdown(cohere_advice)
